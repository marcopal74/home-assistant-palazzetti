"""Provides device triggers for Palazzetti."""
...
TRIGGER_TYPES = {"error_on", "turned_off"}
...
async def async_get_triggers(hass: HomeAssistant, device_id: str) -> List[dict]:
    """List device triggers for Palazzetti devices."""
    ...
    for entry in entity_registry.async_entries_for_device(registry, device_id):
        ...
        if not (entry.unique_id[-6:] == "status" and entry.domain == "sensor"):
            continue
...
async def async_attach_trigger(
    ...
    state_config = {
        ...
        state.CONF_ATTRIBUTE: "_flag_error_status",
        # state.CONF_ATTRIBUTE: "_value_power",
        state.CONF_FROM: from_state,
        state.CONF_TO: to_state,
        # state.CONF_FOR: "00:00:05",
    }
    state_config = state.TRIGGER_SCHEMA(state_config)
    myres = await state.async_attach_trigger(
        hass, state_config, action, automation_info, platform_type="state"
    )
    return myres
